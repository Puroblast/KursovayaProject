    ...
    with auth_db:
        ...
        if len(request) == 6:
            ...
            if flag:
                ...
                with ids_db:
                    curs = ids_db.cursor()
                    curs.execute(f"CREATE TABLE IF NOT EXISTS '{token}' (`mesta` STRING, `otzivi` STRING, 'mark' STRING)")
                    ids_db.commit()
                    curs.close()
            else:
        ...
        with ids_db:
            ...
            for i in range(len(table)):
                if str(table[i][0]) == str(zone_id):
                    ...
                    break
            cur.execute(f"INSERT INTO `{token}` VALUES ('{str(zone_id)}', '{str(call)}', '{mark}')") ## во фронте реализовать невозможность ввода апострофов иначе бум
            ids_db.commit()
            cur.close()
        answer = json.dumps("YES")
        answer = answer.encode('utf-8')
